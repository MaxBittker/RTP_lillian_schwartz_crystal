...
void ofApp::dilate( ofImage & imgSrc, ofImage & imgDest){
    ...
    for (int i = 0; i < imgSrc.getWidth(); i++){
        for (int j = 0; j < imgSrc.getHeight(); j++){
            ...
            int count = 0;
            if ( n_.getBrightness() > 127 ) {count+=1;}
            if ( ne.getBrightness() > 127 ) {count+=1;}
            if ( e_.getBrightness() > 127 ) {count+=1;}
            if ( se.getBrightness() > 127 ) {count+=1;}
            if ( s_.getBrightness() > 127 ) {count+=1;}
            if ( sw.getBrightness() > 127 ) {count+=1;}
            if ( w_.getBrightness() > 127 ) {count+=1;}
            if ( nw.getBrightness() > 127 ) {count+=1;}
